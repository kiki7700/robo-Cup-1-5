    ...
    def spin(self):
        ...
        while not rospy.is_shutdown():
            ...
            if ret == True:
                ...
                #s.sendto(gray_Blur, ('192.168.43.120', 22))
                ...
                if lines_l is None:
                    ...
                    for x0, y0, x1, y1 in lines_l[0]:
                        cv2.line(binary_warped, (x0, y0), (x1, y1), (0, 0, 255),5)  # 画线  binary_warped：背景图 (x0, y0)：直线起始坐标 (x1, y1)：直线终点坐标 (0, 0, 255)：RGB值  5:5个像素宽度
                        y0 = 160 - y0  # 坐标轴转换
                        y1 = 160 - y1
                        if x0 == x1:
                            theta_l = 1.57
                ...
                cam_cmd.linear.x = 0.2
                    ...
                    # if(is_bend==0):  #直道
                steerAngle = steerAngle_calculate_straight(theta_l, theta_r, binary_caculate)
                calculate_z = args.k * steerAngle
                cur_z = 0.875 *  calculate_z + 0.125 * old_z
                cam_cmd.angular.z = cur_z
                old_z = calculate_z
